...
#include "luvk_example/ImGuiBackendLUVK.hpp"
...
{
    m_Renderer = Renderer;
    m_Device = Renderer->FindModule<luvk::Device>();
    m_CommandPool = Renderer->FindModule<luvk::CommandPool>();
    m_Memory = Renderer->FindModule<luvk::Memory>();
    if (!m_Device || !m_CommandPool || !m_Memory)
    {
        return false;
    }
    ...
    m_FontImage->CreateImage(m_Device, m_Memory, {.Extent = {static_cast<uint32_t>(Width), static_cast<uint32_t>(Height), 1},
    ...
    auto Swap = Renderer->FindModule<luvk::SwapChain>();
    if (!Swap)
    {
        return false;
    }
...
void ImGuiBackendLUVK::Render(VkCommandBuffer Cmd) const
{
    ImGui::Render();
    ...
    std::size_t VertexSize = DrawData->TotalVtxCount * sizeof(ImDrawVert);
    std::size_t IndexSize = DrawData->TotalIdxCount * sizeof(ImDrawIdx);
    ...
    VkBuffer VertexBufferHandle = m_VtxBuffer->GetHandle();
    VkBuffer IndexBufferHandle = m_IdxBuffer->GetHandle();
    ...
    vkCmdBindIndexBuffer(Cmd, IndexBufferHandle, 0, sizeof(ImDrawIdx) == 2 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32);

    struct PC { float Scale[2]; float Translate[2]; } PushConst;
    ...
    for (int n = 0; n < DrawData->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < CmdList->CmdBuffer.Size; cmd_i++)
        {
            ...
            VkRect2D Scissor;
            Scissor.offset.x = (int)(Cmd->ClipRect.x - DrawData->DisplayPos.x);
            Scissor.offset.y = (int)(Cmd->ClipRect.y - DrawData->DisplayPos.y);
            ...
            vkCmdDrawIndexed(Cmd, Cmd->ElemCount, 1, Cmd->IdxOffset + IndexOf, Cmd->VtxOffset + VertexOf, 0);
        }
